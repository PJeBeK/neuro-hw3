function [model, train_accr, test_accr, cross_accr, train_word, test_word] = SubjectLDAAnalysisWithRegularization(subject_id)
    %load root
    ...
    % train LDA
    model = fitcdiscr(train_X, train_y, 'Gamma', 0.4);
    ...
    f1.Position(3) = 1400;
    f1.Position(4) = 700;
    all_fig_path = strcat(root, 'output/phase5/Subject', num2str(subject_id), '.png');
    [status, ~, ~] = mkdir(strcat(root,'output'));
    assert(status == 1, 'output directory creation failed');
    [status, ~, ~] = mkdir(strcat(root,'output/phase5/'));
    assert(status == 1, 'output directory creation failed');
